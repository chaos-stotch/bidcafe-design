...
export const TableSeller = () => {
  ...
  const [allItems, setAllitems] = useState(all_infos as IInfos[])
  const [sellers, setSellers] = useState([] as IInfos[])
  ...
  const editSeller = (temp_seller: any) => {
    console.log(temp_seller);
  ...
  const registeSeller = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const url = import.meta.env.VITE_BASE_URL
      const {data} = await axios.post(`${url}/seller`, {
        nome: item.nome,
  ...
  const updateSeller = async () => {
    ...
    try {
      const url = import.meta.env.VITE_BASE_URL
      const {data} = await axios.put(`${url}/seller/${item.id}`, {
        nome: item.nome,
        endereco: item.endereco,
        cidade: item.cidade,
        uf: item.uf,
  ...
  const deleteSeller = async (id: number) => {
    ...
    try {
      const url = import.meta.env.VITE_BASE_URL
      const {data} = await axios.delete(`${url}/seller/${id}`)  
            ...
            {sellers.map((info) => (
              <TableRow key={info.id}>
                ...
                <TableCell className="font-medium flex gap-x-2 !max-w-[200px]">
                  ...
                  <Button className="text-xs bg-red-500" onClick={() => deleteSeller(Number(info.id))}>Deletar</Button>
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
      ...
      <Dialog open={update || create}>
        <DialogContent className="">
          <DialogHeader>
            ...
            <DialogDescription>
            {update ? 'Editar Vendedor': 'Criar Vendedor'}
            </DialogDescription>
            <form onSubmit={registeSeller} className="w-full space-y-3">
